<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="skuList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品价格" align="center" prop="priceVo" />
...
/** 查询所有商品类型下拉框 */
listSkuClass(commonQuery).then(response => {
  classOptions.value = response.rows.map(item => {
    return {
      value: item.classId,
...
function submitForm() {
  proxy.$refs["skuRef"].validate(valid => {
    if (valid) {
      form.value.price = form.value.priceVo * 100;
