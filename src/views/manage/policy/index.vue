  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="policyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary"  @click="handleDetail(scope.row)" v-hasPermi="['manage:policy:edit']">策略详情</el-button>
    ...
    <!-- 策略详情对话框 -->
     <el-dialog :title="title" v-model="detailOpen" width="500px" append-to-body>
      <el-form-item label="策略名称" prop="policyName">
          <el-input v-model="form.policyName" placeholder="请输入策略名称" :disabled="true" />
      </el-form-item>
      <label>包含点位：</label>
      ...
      <el-table-column label="点位地址" align="center" prop="addr" show-overflow-tip = "true" />
...
const data = reactive({
  ...
  rules: {
    ...
    discount: [
      { required: true, message: "策略方案", trigger: "blur" }
    ],
  }
});
...
function handleDetail(row) {
  commonQuery.policyId = row.policyId;
  listVm(commonQuery).then(response => {
